<template>
	<view :class="{ text: styleType === 'text' }" :style="{borderColor: styleType === 'text' ? '' : activeColor}" class="segmented-control">
		<view v-for="(item, index) in values" :key="index" :class="[{ text: styleType === 'text' }, { active: index === currentIndex }]"
		 :style="{
        color:index === currentIndex ? (styleType === 'text'? activeColor : '#fff') : (styleType === 'text' ? activeColor : '#b0b0b9'),
